# PRO10 Windows Bridge v0.6 - cache persistente de clima
...
LOG_PATH = BASE_DIR / "pro10_bridge_v0_6.log"
BATTERY_CSV = BASE_DIR / "pro10_bateria.csv"
WEATHER_CACHE_PATH = BASE_DIR / "pro10_clima_cache.json"
...
def weather_condition(wmo):
    ...
    return conditions.get(wmo, "Condicao desconhecida")


def validate_weather(weather):
    if not isinstance(weather, dict):
        raise ValueError("resposta de clima nao e um objeto")
    current = weather.get("current")
    daily = weather.get("daily")
    if not isinstance(current, dict) or not isinstance(daily, dict):
        raise ValueError("current/daily ausentes")
    ...
    for field in fields:
        if not isinstance(daily.get(field), list) or len(daily[field]) < 7:
            raise ValueError(f"previsao de 7 dias incompleta: {field}")

    for index in range(7):
        datetime.strptime(str(daily["time"][index])[:10], "%Y-%m-%d")
...
def weather_cache_record(weather):
    weather = validate_weather(weather)
    current = weather["current"]
    daily = weather["daily"]
    current_wmo = int(current["weather_code"])
    forecast = []
    for index in range(7):
        wmo = int(daily["weather_code"][index])
        forecast.append(
            {
                "data": str(daily["time"][index])[:10],
                "temperatura_minima": float(daily["temperature_2m_min"][index]),
                "temperatura_maxima": float(daily["temperature_2m_max"][index]),
                "codigo_wmo": wmo,
                "condicao": weather_condition(wmo),
            }
        )
    return {
        "versao_cache": 1,
        "cidade": str(CFG["city"]),
        "temperatura_atual": float(current["temperature_2m"]),
        "sensacao": float(current["apparent_temperature"]),
        "condicao": weather_condition(current_wmo),
        "codigo_wmo": current_wmo,
...
def save_weather_cache(weather):
    record = weather_cache_record(weather)
    temporary_path = WEATHER_CACHE_PATH.with_name(WEATHER_CACHE_PATH.name + ".tmp")
    temporary_path.write_text(
        json.dumps(record, ensure_ascii=False, indent=2), encoding="utf-8"
    )
    os.replace(temporary_path, WEATHER_CACHE_PATH)
    return record


def load_weather_cache():
    record = json.loads(WEATHER_CACHE_PATH.read_text(encoding="utf-8"))
    required = (
        "cidade",
        "temperatura_atual",
        "sensacao",
    ...
    if any(field not in record for field in required):
        raise ValueError("cache sem todos os campos obrigatorios")
    if not isinstance(record["previsao_7_dias"], list) or len(
        record["previsao_7_dias"]
    ) != 7:
        raise ValueError("cache sem previsao completa de 7 dias")
    datetime.fromisoformat(str(record["atualizado_em"]))
    weather = validate_weather(record["dados_clima"])
    return weather, record


def get_weather_online():
    params = {
    ...
    with urllib.request.urlopen(request, timeout=12) as response:
        return validate_weather(json.loads(response.read().decode("utf-8")))


def get_weather():
    try:
        weather = get_weather_online()
    except Exception as online_error:
        LOG.warning(
            "[CLIMA] Consulta online falhou (%s): %s",
            type(online_error).__name__,
            online_error,
        )
        try:
            weather, record = load_weather_cache()
        except Exception as cache_error:
            raise RuntimeError(
                "consulta online indisponivel e cache valido nao encontrado"
            ) from cache_error
        LOG.info(
            "[CLIMA] Origem=cache | cidade=%s | atualizado=%s",
            record["cidade"],
    ...
    try:
        record = save_weather_cache(weather)
    except Exception as cache_error:
        LOG.warning(
            "[CLIMA] Internet respondeu, mas o cache nao foi salvo (%s): %s",
            type(cache_error).__name__,
            cache_error,
        )
    else:
        LOG.info(
            "[CLIMA] Origem=internet | cidade=%s | cache_atualizado=%s",
            record["cidade"],
            record["atualizado_em"],
        )
    return weather
...
class Bridge:
    ...
    async def run_forever(self):
        LOG.info("=" * 60)
        LOG.info("PRO10 v0.6 - cache persistente + clima/hora + bateria")
